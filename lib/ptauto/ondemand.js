(function() {
	...
	const onDemandCategories = async (config, region, bootData) => {
		...
		const resp = await axios.get(`https://${VOD_HOST}/v4/vod/categories?includeItems=false&includeCategoryFields=iconSvg&offset=1000&page=1&sort=number%3Aasc`, {headers});
	...
	const getItems = async (config, region, categoryID, bootData) => {
		...
		const resp = await axios.get(`https://${VOD_HOST}/v4/vod/categories/${categoryID}/items?offset=0&page=1`, {headers});
		return resp.data;
	}
	...
	const getVodItem = async (config, region, id, bootData) => {
		...
		const resp = await axios.get(`https://${VOD_HOST}/v4/vod/items?ids=${id}`, {headers});
	...
	const generateM3U8 = async (config, region, bootData) => {
		...
		if (xTvgUrl) fullTvgUrl = xTvgUrl + (xTvgUrl.endsWith('/') ? `ptauto_ondemand_${region}.xml` : '');
		...
		const topTitlesCategory = categories.find((c) => `${c?.name || ''}`.trim().toLowerCase() === 'top titles');
		if (!topTitlesCategory) {
			console.log('Top Titles category not found, skipping ondemand generation');
			return { m3u8, numChannels };
		}

		let cacheDirty = false;
		const c = topTitlesCategory;
		console.log("---------------------------", c.name, 0, categories.length);
